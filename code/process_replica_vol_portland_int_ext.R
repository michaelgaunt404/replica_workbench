...
network_link_volumes_pro_comb = 
  c('data/geo_manual_tracts_20220811/internal_trips'
    ,'data/geo_manual_tracts_20220811/external_trips') %>% 
  map(~{
    network_link_volumes_raw = read_csv_allFiles2(
      data_location = .x
      ,specifically = "network_vols", latest = F, clean_string  = ".csv")
    
    network_link_volumes_pro = process_replica_network(network_link_volumes_raw)
  }) %>%  
  reduce(bind_rows)

network_link_volumes_pro_int = network_link_volumes_pro_comb %>% 
  filter(source != "network_vols_external") %>% 
...
network_link_volumes_pro_ext = network_link_volumes_pro_comb %>% 
  filter(source == "network_vols_external") %>% 
  select(stableEdgeId_adj, direction, starts_with("vol"), source) 

network_link_volumes_pro_ext_sf = network_link_volumes_pro_ext %>% 
  merge(network, .
        ,by = "stableEdgeId_adj")
...
network_link_volumes_pro_comb_sf %>%  
  filter(volume_rnk >= .7
         ,source == "network_vols_external") %>% 
    mapview(zcol = "volume_rnk_binc", burst = T, homebutton = F)

network_link_volumes_pro_ext_sf %>%  mapview()
